    ...
    <>
      <div className="flex gap-1.5 shrink-0">
        {files.map((m, i) => (
          ...
          >
            {m.type === 'image' ? (
              <img src={m.url} alt={`M\u00eddia ${i + 1}`} className={`${thumbnailClass} rounded-lg object-cover group-hover:opacity-80 transition-opacity`} />
            ) : (
              <div className="relative">
                <video src={m.url} className={`${thumbnailClass} rounded-lg object-cover`} />
